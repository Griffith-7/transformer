...
def generate_synthetic_corpus():
    ...
    adjs = [
        w
        for w in vocab
        if w in ("big", "small", "long", "short", "old", "new", "good", "bad", "great", "little")
    ]
    verbs = [
        w
        for w in vocab
...
def train_model(name, model_cls, tokenizer):
    ...
    while step < TOTAL_STEPS:
        for x, y in train_loader:
            ...
            if step % 50 == 0:
                print(
                    f"  Step {step:4d}/{TOTAL_STEPS} | Loss: {loss.item():.4f} | LR: {LR * lr_scale:.2e}"
                )
    ...
    return {
        "name": name,
        "params": params,
        "final_loss": sum(train_losses[-50:]) / 50,
...
def main():
    ...
    for r in results:
        ...
        math.exp(r["final_loss"]) if r["final_loss"] < 10 else 999
        print(
            f"  {name_short + ' (' + name_short + ')':<25} {r['params'] / 1e6:>7.2f}M {r['final_loss']:>12.4f} {r['best_val_loss']:>12.4f} {r['elapsed']:>7.1f}s"
        )
    print()
